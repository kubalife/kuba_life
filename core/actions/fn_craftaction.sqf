...
if(!((player distance (getMarkerPos "CraftingArea1") < 50) OR  (player distance (getMarkerPos "CraftingArea2") < 50) OR  (player distance (getMarkerPos "CraftingArea3") < 50))) exitWith  {hint "Um das zu machen brauchst du Werkzeuge, geh in eine Crafting Zone!";};
...
//Some checks
if((count _matsNeed) == 0) exitWith { hint "Du hast nicht alle benötigten Materialien.";};
...
if(_itemFilter == "vest" && vest player != "") exitwith {
        hint "Du musst deine Weste ablegen bevor du eine neue craften kannst!";
};
...
if(_itemFilter == "vest") then{
    if(vest player == "") then{
        ...
        hint "Du trägst bereits eine Weste";
        life_is_processing = false;
    };
};
...
if(_itemFilter == "weapon") then{
	...
	if(player canAdd _newItem) then{
		...
		if(currentWeapon player == "") then{
			...
			5 cutText ["","PLAIN"];
			hint "Du trägst bereits eine Waffe, lege sie in den Rucksack und versuche es erneut!";
			for [{_i=0},{_i<_invSize-1},{_i=_i+2}] do {
				_handledItem = M_CONFIG(getText,"VirtualItems",_oldItem select _i,"variable");
