...
_vehicle = createVehicle ["O_Truck_03_device_F", position player, [], 0, "NONE"];
...
[false,"golf",1] call life_fnc_handleInv; 


/*
	_vehicle = createVehicle ["C_Quadbike_01_F", position player, [], 0, "NONE"];
	waitUntil {!isNil "_vehicle" && {!isNull _vehicle}}; //Wait?
	_vehicle allowDamage false; //Temp disable damage handling..
    _vehicle lock 2;
    _vehicle setVectorUp (surfaceNormal position player);
	...
	[getPlayerUID player,playerSide,_vehicle,1] remoteExecCall ["TON_fnc_keyManagement",RSERV];
	[(getPlayerUID player),playerSide,_vehicle] remoteExecCall ["TON_fnc_vehicleCreate",RSERV];
	
[0] call SOCK_fnc_updatePartial;
*/

/*
    _vehicle = createVehicle ["cl3_golf_mk2_aqua", position player, [], 0, "NONE"];
